...
import { IAddFurniture, IAddFurnitureValidator, IUploadedPhoto } from './dto/IAddFurniture'; // ovo nzm sta ti je
...
import { IEditFurniture, IEditFurnitureValidator } from './dto/IEditFurniture';
...
class FurnitureController extends BaseController {
    public async getById(req: Request, res: Response) {
        ...
        const item = await this.services.furnitureService.getById(
            id,
            {
                loadCategory: true,
                loadPrices: true,
    ...
    public async add(req: Request, res: Response) {
        ...
        try {
            ...
            if (!IAddFurnitureValidator(data)) {
                res.status(400).send(IAddFurnitureValidator.errors);
                return;
            }

            const result = await this.services.furnitureService.add(data as IAddFurniture, uploadedPhotos);
    ...
    public async edit(req: Request, res: Response) {
        ...
        if (!IEditFurnitureValidator(req.body)) {
            return res.status(400).send(IEditFurnitureValidator.errors);
        }

        const result = await this.services.furnitureService.edit(id, req.body as IEditFurniture);
    ...
    public async delete(req: Request, res: Response) {
        ...
        const item = await this.services.furnitureService.getById(id);
        ...
        res.send(await this.services.furnitureService.delete(id));
    }

    public async deleteFurniturePhoto(req: Request, res: Response) {
        const furnitureId: number = +(req.params?.aid);
        const photoId: number   = +(req.params?.pid);

        if (furnitureId <= 0 || photoId <= 0) return res.sendStatus(400);

        const result = await this.services.furnitureService.deleteFurniturePhoto(furnitureId, photoId);
    ...
    public async addFurniturePhotos(req: Request, res: Response) {
        const furnitureId: number = +(req.params?.id);

        if (furnitureId <= 0) return res.sendStatus(400);

        const item = await this.services.furnitureService.getById(furnitureId);
        ...
        res.send(await this.services.furnitureService.addFurniturePhotos(furnitureId, uploadedPhotos));
    }
