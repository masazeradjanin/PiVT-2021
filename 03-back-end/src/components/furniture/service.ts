...
private async getAllPricesByFurnitureId(furnitureId: number): Promise<FurniturePrice[]> {
    const sql = `
        SELECT
            furniture_price_id,
            created_at,
...
private async getAllPhotosByFurnitureId(furnitureId: number): Promise<FurniturePhoto[]> {
...
private async getLatestPriceByFurnitureId(furnitureId: number): Promise<number> {
        ...
        .then(() => {
            ...
            .then(async (res: any) => {
                ...
                .then(async () => {
                    ...
                    resolve(await this.services.furnitureService.getById(
                        newFurnitureId,
                        {
                            loadCategory: true,
                            loadPhotos: true,
...
public async edit(furnitureId: number, data: IEditFurniture): Promise<FurnitureModel|null|IErrorResponse> {
    return new Promise<FurnitureModel|null|IErrorResponse>(async resolve => {
        ...
        this.db.beginTransaction()
            .then(() => {
                this.editFurniture(furnitureId, data)
                .catch(error => {
                    rollbackAndResolve({
                        errno: error?.errno,
            ...
            .then(async () => {
                const currentPrice = (currentFurniture as FurnitureModel).currentPrice.toFixed(2);
                const newPrice     = data.price.toFixed(2);

                if (currentPrice !== newPrice) {
                    this.addFurniturePrice(furnitureId, data.price)
                    .catch(error => {
                        rollbackAndResolve({
                            errno: error?.errno,
            ...
            .then(async () => {
                resolve(await this.getById(furnitureId, {
                    loadCategory: true,
                    loadPhotos: true,
                    loadPrices: true,
                }));
...
public async delete(furnitureId: number): Promise<IErrorResponse|null> {
    return new Promise<IErrorResponse>(async resolve => {
        ...
        this.db.beginTransaction()
            ...
            .then(async () => {
                const filesToDelete = await this.deleteFurniturePhotoRecords(furnitureId);
            ...
            .then(async (filesToDelete) => {
                if (await this.deleteFurnitureRecord(furnitureId)) return filesToDelete;
...
private async deleteFurniturePrices(furnitureId: number): Promise<boolean> {
    return new Promise<boolean>(async resolve => {
        this.db.execute(
            `DELETE FROM furniture_price WHERE furniture_id = ?;`,
            [ furnitureId ]
        )
        .then(() => resolve(true))
        .catch(() => resolve(false));
...
private async deleteFurnitureCartRecord(furnitureId: number): Promise<boolean> {
    return new Promise<boolean>(async resolve => {
        this.db.execute(
            `DELETE FROM cart_furniture WHERE furniture_id = ?;`,
            [ furnitureId ]
        )
        .then(() => resolve(true))
        .catch(() => resolve(false));
...
private async deleteFurniturePhotoRecords(furnitureId: number): Promise<string[]> {
    return new Promise<string[]>(async resolve => {
        ...
        this.db.execute(
            `DELETE FROM photo WHERE furniture_id = ?;`,
            [ furnitureId ]
        )
...
public async deleteFurniturePhoto(furnitureId: number, photoId: number): Promise<IErrorResponse|null> {
    return new Promise<IErrorResponse|null>(async resolve => {
        ...
        const filteredPhotos = (furniture as FurnitureModel).photos.filter(p => p.photoId === photoId);
        ...
        .then(() => {
            this.deleteFurniturePhotosAndResizedVersion([
                photo.imagePath
            ]);
...
public async addFurniturePhotos(furnitureId: number, uploadedPhotos: IUploadedPhoto[]): Promise<FurnitureModel|IErrorResponse|null> {
    return new Promise<FurnitureModel|IErrorResponse|null>(async resolve => {
        const furniture = await this.getById(furnitureId, {
            loadPhotos: true,
        });
...
public async getAllByCategoryId(categoryId: number): Promise<FurnitureModel[]> {
    return await this.getAllByFieldNameFromTable<FurnitureModelAdapterOptions>("furniture", "category_id", categoryId, {
        loadPhotos: true,
    }) as FurnitureModel[];
}
}
