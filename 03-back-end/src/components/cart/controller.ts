...
export default class CartController extends BaseController {
    ...
    public async addToCart(req: Request, res: Response) {
        ...
        const furniture = await this.services.furnitureService.getById(data.furnitureId);

        if (furniture === null) {
            return res.status(404).send("Furniture is not found.");
        }

        res.send(
            await this.services.cartService.addFurnitureToLatestCartByUserId(
                req.authorized?.id,
                data.furnitureId,
                data.quantity
            )
    ...
    public async setInCart(req: Request, res: Response) {
        ...
        res.send(
            await this.services.cartService.setFurnitureToLatestCartByUserId(
                req.authorized?.id,
                data.furnitureId,
                data.quantity
            )
